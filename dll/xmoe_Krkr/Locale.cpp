...
{
	if (CodePage == 0 && dwFlags == MB_PRECOMPOSED && lpMultiByteStr != nullptr &&
		cbMultiByte != -1 && lpWideCharStr && cchWideChar)
	{
		return MultiByteToWideChar(932,
			dwFlags,
			lpMultiByteStr,
			cbMultiByte,
			lpWideCharStr,
	...
	*(FARPROC *)&pfTVPGetFunctionExporter = GetProcAddress(hModule, "TVPGetFunctionExporter");
	...
	if (pfTVPGetFunctionExporter != nullptr)
	{
		...
		*/
		...
		FARPROC pfCreateFontIndirectA = GetProcAddress(GetModuleHandleW(L"Gdi32.dll"), "CreateFontIndirectA");

		//FARPROC pfMultiByteToWideChar = GetProcAddress(GetModuleHandleW(L"Kernel32.dll"), "MultiByteToWideChar");
		...
		IATPatch("Gdi32.dll", pfCreateFontIndirectA, (PROC)HookCreateFontIndirectA);
		//IATPatch("Kernel32.dll", pfMultiByteToWideChar, (PROC)HookMultiByteToWideChar);
